  ...
  <div class="trip-panels" id="{{ $tabsId.'-panel' }}" aria-live="polite">
    <section class="trip-panel" data-trip-panel="discount" role="tabpanel" aria-labelledby="{{ $tabsId.'-discount' }}">
      @if($discountPercent > 0)
        <div class="trip-price-grid"><div><span>{{ $homeTripCopy['original'] }}</span><del>{{ number_format($originalFare) }} VND</del></div><div><span>{{ $homeTripCopy['sale'] }}</span><strong>{{ number_format($trip['fare']) }} VND</strong></div><div class="trip-price-save"><b>-{{ $discountPercent }}%</b><span>{{ $homeTripCopy['save'] }} {{ number_format($originalFare - $trip['fare']) }} VND</span></div></div>
      @else
