            ...
            <div class="route-grid">
                @foreach($routes as $route)
                    ...
                    <article class="route-card">
                        <div class="route-card__visual" aria-label="{{ $from }} → {{ $to }}"><span class="route-card__status">{{ $copy['available'] }}</span><div class="route-card__path"><span>{{ $from }}</span><span>{{ $to }}</span></div></div>
                        <div class="route-card__body">
                            <h3>{{ $from }} <span>{{ $copy['connector'] }}</span> {{ $to }}</h3>
