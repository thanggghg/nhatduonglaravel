...
@section('content')
<section class="booking-success"><div class="success-card"><p class="success-mark">OK</p><h1>{{ $copy['title'] }}</h1><p>{{ $copy['text'] }}</p><div class="success-reference"><span>{{ $copy['reference'] }}</span><strong>{{ $booking->reference }}</strong></div><dl><div><dt>{{ $copy['status'] }}</dt><dd>{{ $booking->payment_status === 'paid' ? $copy['paid'] : $copy['pending'] }}</dd></div><div><dt>{{ $copy['trip'] }}</dt><dd>{{ $booking->travel_date->format('d/m/Y') }} · {{ $departure?->format('H:i') }} · {{ $vehicleType }}</dd></div>@if($booking->returnSchedule)<div><dt>{{ $copy['return'] }}</dt><dd>{{ $booking->return_travel_date->format('d/m/Y') }} · {{ $booking->returnSchedule->departure_time?->format('H:i') }}</dd></div>@endif<div><dt>{{ $copy['total'] }}</dt><dd>{{ number_format($booking->total_amount) }} VND</dd></div></dl><div class="success-actions"><a href="{{ route('home', ['lang' => $locale]) }}">{{ $copy['home'] }}</a><a href="{{ route('contact', ['lang' => $locale]) }}">{{ $copy['help'] }}</a></div></div></section>
@endsection
